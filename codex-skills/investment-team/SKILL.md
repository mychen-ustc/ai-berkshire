...
**可选异见角色（第 5-6 席）——用于对冲四大师"同源思维"的集体盲点：**

| 角色 | 职责 | 分析框架 | 何时启用 |
|------|------|----------|---------|
| **cycle-risk-officer** | 周期定位 & 情绪拥挤度 & 生存/仓位 | 霍华德·马克斯 + 塔勒布 + 帕伯莱 | **A 级共识股强制**；其余按需 |
| **macro-analyst** | 宏观机器 & 组合相关性 & 分散 | 瑞·达利欧 | **宏观敏感股（金融/地产/出口/中概）强制**；其余按需 |

> 四大师同属"质量价值一脉"，想法同源，容易在同一处集体犯错（高位重仓、忽略周期/宏观/尾部风险）。异见角色的作用是**唱反调**——尤其对券商密集覆盖、市场已充分定价的 A 级股，请进马克斯（周期）和塔勒布（生存）能避免输出"正确的废话"。完整大师库见 `skills/master-lens.md`。
...
| 等级 | 特征 | 研究策略调整 |
|------|------|------------|
| A级（信息充裕） | 上市多年、券商覆盖广 | 团队重点放在**反面检验**和**非共识视角**，避免输出与市场一致的"正确的废话"；**强制启用第 5-6 席异见角色**（cycle-risk-officer 必启，macro-analyst 视宏观敏感度启用） |
...
#### 任务5（可选 / A级共识股必启）：周期定位与生存仓位
- subject: `定位{公司名}所处市场周期、情绪位置与生存底线`
- 分析框架：霍华德·马克斯 + 塔勒布 + 帕伯莱
- description 包含：
  1. 情绪钟摆：当前行业/标的情绪偏贪婪还是恐惧
  2. 估值历史分位（与 financial-analyst 数据交叉）
  3. 共识拥挤度：卖方评级、南向/北向资金、社媒热度是否一边倒（拥挤多头=风险，拥挤空头=机会）
  4. 第二层思维：市场共识是什么？我的判断比共识深在哪？
  5. **塔勒布生存测试**：最坏情况（造假/监管归零/技术颠覆/地缘）会不会让组合永久出局？
  6. **帕伯莱非对称赔率**：用三情景结果算 上行空间 ÷ 下行空间，并给出建议仓位区间（凯利精神）
  7. 要求搜索最新资金流、情绪指标、卖方一致预期

#### 任务6（可选 / 宏观敏感股必启）：宏观与组合视角
- subject: `评估宏观环境与组合层面对{公司名}的影响`
- 分析框架：瑞·达利欧
- description 包含：
  1. 宏观机器：利率/汇率/信贷/债务周期站在它哪一边
  2. 政策与监管周期定位
  3. 相关性：这笔投资是否与常见持仓押注同一个宏观逻辑（重复暴露）
  4. 分散视角：它给组合带来的是新的独立回报流，还是又一个相关的赌注
  5. 全天候思考：滞胀/衰退/复苏/过热四种宏观情景下分别如何表现
  6. 要求搜索最新宏观数据、政策动态

### 第四步：启动并行Agent（4 核心 + 按需 2 异见）

使用 Task 工具在**同一条消息中并行调用**：核心 4 个 Agent 恒启动；若第一步半判定为 A 级共识股或宏观敏感股，则**追加启动**对应异见 Agent（共 5 或 6 个）。

每个Agent的配置：
- `subagent_type`: `general-purpose`
- `run_in_background`: `true`
- `team_name`: 对应团队名
- `name`: 对应角色名（核心：business-analyst / financial-analyst / industry-researcher / risk-assessor；异见：cycle-risk-officer / macro-analyst）

> 异见 Agent 同样适用下方 prompt 模板与"联网失败禁止伪装"规则；其 {大师名} 分别为"马克斯+塔勒布+帕伯莱""达利欧"，{任务编号} 为 5 / 6。
...
- 等待全部报告到齐（核心 4 份；启用异见角色时 5-6 份）

### 第六步：关闭团队成员

全部报告收到后，向全部 Agent（核心 4 个 + 已启用的异见角色）发送 shutdown_request（使用 SendMessage，type: "shutdown_request"）。
...
核心四维恒列（段永平/巴菲特/芒格/李录）；启用异见角色时**追加行**：周期定位（马克斯）、生存与仓位（塔勒布+帕伯莱）、宏观与组合（达利欧）。
...
1. **核心 4 个 Agent 必须并行启动**——在同一条消息中调用 Task 工具；A级共识股/宏观敏感股在同一条消息中**追加启动**异见 Agent（cycle-risk-officer / macro-analyst），共 5-6 个
