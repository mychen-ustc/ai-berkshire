...
- 如果判断有误，在当前价格买入最多亏多少？
- 股价腰斩你敢加仓吗？
- **赔率检验（帕伯莱）**：上行空间 ÷ 下行空间是否明显 >1（非对称向上）？向下不对称的直接扣分。
- **vs 指数化（博格）**：这只股票的预期回报凭什么跑得赢对应低成本宽基指数？说不清就该老实指数化。
...
- [ ] 无法承受这笔投资归零的后果
- [ ] **（塔勒布·生存）最坏情况下，这笔的仓位会让整个组合永久出局——不只是这一笔亏光，而是下不了牌桌**
- [ ] **（帕伯莱·赔率）下行空间大于上行空间，赔率向下不对称**
...
- **先活下来（塔勒布）**：任何单一标的都不能让组合面临永久出局风险，仓位是生命线
- **跑不赢指数就指数化（博格）**：没有清晰信息优势时，低成本宽基指数才是默认答案
