...
第三层：精细分析          ≤ 10 家  （每家 300-500 字结构化分析）
        ↓ 终选
第四层：13 位大师深度分析   3 家     （每家 800-1200 字，4 核心 + 9 国际，默认全用）
        ↓
输出：投资建议 + 操作信号 + 仓位建议
```
...
## 第四步：13 位大师深度分析（3 家，每家 800-1200 字；4 核心 + 9 国际）

对终选 3 家执行 13 位大师视角深度分析（4 核心深读 + 9 国际唱反调，见 CLAUDE.md「投资大师阵容」；不默认只用四大师）。
...
漏斗终选 3 家后，对每家可单独执行：
- `/investment-team` —— 完整 13 席（4 核心+9 国际）并行深度研究（独立子目录 + 文档）
