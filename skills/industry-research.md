# 行业投资研究：产业链全景扫描 + 13 位大师个股分析框架（4 核心 + 9 国际，默认全用）
...
4. 对每个细分环节的头部公司执行 13 位大师框架分析（4 核心 + 9 国际唱反调，不默认只用四大师）
5. 输出行业级投资组合配置建议
...
## 第四步：各环节头部公司 13 位大师分析（4 核心 + 9 国际）
...
### 大师模拟点评（默认 13 位，见 CLAUDE.md「投资大师阵容」）
用引用格式，模拟大师对该行业投资机会的点评（默认 13 席按相关性选，不默认只用四大师）。
