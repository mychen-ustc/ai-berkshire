...
# 周一 06:45 摄取(复盘前刷新);run_audit 留痕(run-id/SHA/结果可追溯复现);失败即告警
INGEST="45 6 * * 1 cd '$ROOT' && { '$PY' tools/run_audit.py wrap --trigger cron-ingest -- '$PY' tools/ingest_universe.py run >> '$ILOG' 2>&1 || '$PY' tools/notify.py send --title '数据摄取失败' --message 'ingest_universe run 非零退出,见 logs/ingest-cron.log' --level crit; } $TAG"
# 工作日 07:15 健康体检(run_audit 留痕;--notify:非🟢时发桌面/webhook/日志告警;非零退出=有🔴)
MONITOR="15 7 * * 1-5 cd '$ROOT' && '$PY' tools/run_audit.py wrap --trigger cron-monitor -- '$PY' tools/monitor.py check --notify >> '$MLOG' 2>&1 $TAG"
